...
| **Crescent Consultancy** (Mirza Asad Raza, CEO) | `_context/clients/crescent-consultancy/` | **Lead, requirements response sent 2026-08-02.** Different product, not the POS — a back-office inventory & cost-accounting system for restaurants (their team keeps books for multiple restaurant clients). Waiting on Mirza's answers to 12 questions before scope/price is final. Nothing quoted yet. |
| **FZ LLC** (Martin Zubeldia) | `_context/clients/fz-llc-uae/` | **Lead, 2026-08-26.** UAE bakery/restaurant, no dine-in. Wants web POS + Inventory + Procurement + multi-layer recipe/sub-recipe production + OCR goods receiving + AI-assisted PO suggestions + per-sales-channel net-profitability reporting. Trading name not yet confirmed. Video walkthrough being transcribed, delivery-platform/payment-gateway research in progress. |
