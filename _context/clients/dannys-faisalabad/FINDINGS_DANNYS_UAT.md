...
| D-67 | Cash drawer close (all tenants) | Expected cash subtracts change twice: payments store the bill amount (`amount`) with change separate, and `payment_service._calculate_expected_drawer_balance` subtracts `change_amount` again, so every cash sale needing change understates "expected" and reports the drawer short. Verified in code on all payment paths (single, split, session, session split). | Double count. | High (money, every tenant with a drawer) | DEPLOYED `2134b0a` 2026-09-26 (browser close not yet walked; no usable drawer screen, see D-73): change no longer subtracted in `payment_service._calculate_expected_drawer_balance`, the Z-Report drawer block, or the demo seed; D-60 cash position already correct. Test `test_change_given_is_not_counted_twice` passes, and fails with 505000 (Rs 50 short) when the old line is put back. Its first failure (drawer saw no sale) was a SQLite test artifact: second-resolution TEXT timestamps vs a bound `.000000`, so a same-second payment fell outside the window; Postgres unaffected; test opens the drawer 1 s earlier. Full suite 11 failed + 2 errors, all pre-existing. Past closed sessions keep their stored figures. |
| (deploy) | Round 4, 2026-09-26 ~06:45 PKT | D-61..D-67 DEPLOYED in `2134b0a` (run 36209126033 success; prod backup `pre_migrate_2026-09-26_014153.sql` 6.4 MB; live bundle `index-CgbiyaMZ.js` = local build). Prod check `_files/2026-09-26/check_round4_prod.py`: 0 failures (new tables answer, Z-Report has `other_cash_in`/`cash_in_hand`, same-figure opening count = 0 movements on Basmati Rice 39.4 kg, "Admin" expense category added for Danny's). **Browser NOT yet clicked.** | - | - | DEPLOYED, browser pending |
| (pass) | D-61 Opening Stock dialog (browser, 2026-09-26) | PASS: opens with every item, in-system figure, counted box, cost; stock value Rs 868,439. Malik asked: what date is the count booked on, and does it carry forward? Answer: no date field; booked at the moment of saving; stock is one running balance, so every later sale and GRN moves from it and each day's "stock left" is worked back from it. Caveat: a count saved after trading started overwrites sales made between counting and saving. | - | - | PASS, question answered |
| (pass) | D-61 save (browser, 2026-09-26 06:47 PKT) | PASS: Black Pepper 1.94 -> 2 kg saved by Danny's Owner; prod API shows on hand 2.0 kg, one `opening` movement +0.06 -> 2.0, ref OPENING-260926 (`_files/2026-09-26/check_pepper_count.py`). | - | - | PASS |
| (pass) | D-63 Other Income (browser, 2026-09-26) | PASS: Rs 500 cash, Scrap Sale, 26 Sept 2026, recorded by Danny's Owner; totals Rs 500 / cash Rs 500 / bank Rs 0; toast "Income recorded". Opening cash card shows "not set". | - | - | PASS |
| (pass) | D-63 on Z-Report + D-65 wording (browser, 26 Sep) | PASS: Cash taken Rs 3,305 + Other cash income Rs 500 (Scrap sale line) = Net cash Rs 3,805 (was 3,304.84 on the API before the income). "Cash in hand ... once opening cash is recorded" hint shown. Sales by Channel reads "1 order \| Rs. 3,305". | - | - | PASS |
| (pass) | D-62 opening cash on Z-Report (browser, 26 Sep) | PASS: opening cash Rs 10,000 at start of 26 Sep; Z-Report shows start of day Rs 10,000, expected end of day Rs 13,805 (= 10,000 + net cash 3,805), "From opening cash of Rs. 10,000 on 26 Sept 2026". Demo tenant now carries this opening balance. | - | - | PASS |
| (pass) | D-65 dashboard (browser, 26 Sep 07:03) | PASS: "Updated 07:03:47" 24-hour; AOV compares normally with 1 order (Down 52% vs Rs 6,894 to 07:03). Table Utilization 0% is correct with 0 active orders; D-66 still needs an occupied table to prove. | - | - | PASS (D-66 pending) |
| D-73 | Cash drawer has no usable screen (Malik: "where is the drawer, how are we opening it?", 2026-09-26) | The only open/close buttons are a small "Cash Drawer" card on the single-order Payment page (`/payment/:orderId`, `PaymentPage.tsx:285-313`), and they send a FIXED opening float of 0 and a FIXED counted amount of 0: the cashier is never asked either figure. Dine-in "Settle Table" uses `SessionPaymentPage`, which has no drawer card at all, so on Danny's (dine-in) the drawer is never seen. So D-67's fix is correct but invisible, and any close reports the whole day as short. Double-checked 2026-09-26 at Malik's request ("there was a cash drawer already"): that card is the one he used; it exists since `28f5727` (phase 7); no drawer screen was ever deleted (git history); the original demo restaurant showed a real drawer only because `seed.py` inserts a session with a Rs 5,000 float counted Rs 50 short directly into the DB. History (relayed by session pos-project-ff, checked in the files 2026-09-26): BPO World UAT Session 4 (1 Mar 2026) marked UAT-054 "Cash drawer open/close" PASS (`UAT_SESSION_4_SUMMARY.md:45`), but its test (`UAT_CHECKLIST.md:153`) only checked that the status toggles, never a float or a count. The same session logged ENH-007 "Cash drawer session metrics & shift reports" (Medium, `UAT_SESSION_4_SUMMARY.md:147`; `CLIENT_UPDATE_2026-03-01.md:101`: "shift handover reports, over/short variance"), never built. So D-73 is that unbuilt ENH-007, not a regression. Note: `ENHANCEMENT_BACKLOG.md:106` reuses the ID ENH-007 for an unrelated item (Kitchen View shortcut) and carries no drawer entry. | UI never built past a stub (ENH-007, Mar 2026, never built). | High (cash control, all tenants) | FIXED LOCALLY 2026-09-26 (not deployed), with D-68: "Drawer" button in the POS header (all channels incl. dine-in, green dot when open) opens `CashDrawerDialog`: open asks the float; close shows float, + cash sales, - refunds, - cash expenses today, + other cash income today, = should be in the drawer, then the counted amount with live over/short and a confirm step, then a result screen. New `GET /payments/drawer/summary`. Drawer close now takes its expected figure from the Z-Report's own function (`zreport_service.drawer_breakdown` / `_drawers_for_day`); the separate `_calculate_expected_drawer_balance` is gone. The stub card on `PaymentPage` is removed. Test `tests/test_dannys_drawer.py` (3), mutation-checked: the old close rule gives 510000 vs 840000. Visible to every tenant (Martin too). Browser pending. |
| D-68 | Drawer close vs Z-Report drawer (found in code, 2026-09-26) | The two "expected in drawer" figures use different rules. Drawer close (`payment_service._calculate_expected_drawer_balance`): float + cash sales - refunds. Z-Report drawer block (D-60): the same minus the day's cash expenses plus (D-63) cash other income, charged to the last drawer of the day. On a day with a cash expense or cash income the close screen and the Z-Report disagree. | Two formulas for one figure. Expenses and income carry a date, not a time, so which session they belong to is a rule to choose, not a bug to patch. | Medium | DECIDED (Malik, 2026-09-26): YES, the day's cash expenses and cash other income count against the drawer at close, so the close screen and the Z-Report show one figure. FIXED LOCALLY with D-73 (not deployed): drawer close calls the Z-Report's drawer function, one formula. Edge: expenses/income carry a date only, so with two drawers in one day the Z-Report charges them to the later drawer, and an earlier drawer's close figure can then differ from its Z-Report line. |
| D-69 | P&L Statement (Malik, 2026-09-26, verbal) | Monthly profit and loss. | Not built. Feasible from data already held: sales net of tax, cost of goods from recipe consumption, expenses by category, other income. | High (owner-facing) | OPEN, NEW, not scoped |
| D-70 | Trial Balance (Malik, 2026-09-26, verbal) | Trial balance. | Not built, and the system has no general ledger or chart of accounts: sales, stock, expenses and cash are separate tables, not double-entry journals. A true TB needs a ledger (the QuickBooks sync maps into QB's own). | - | OPEN, NEW, needs a scoping decision |
| D-71 | Balance Sheet (Malik, 2026-09-26, verbal) | Balance sheet. | Same gap as D-70: assets/liabilities/equity need a ledger, opening balances for every account (not only cash, D-62), payables and receivables. | - | OPEN, NEW, needs a scoping decision |
| D-72 | Item-wise monthly analysis (Malik, 2026-09-26, verbal) | "What each item cost in the month, vs how much of it was sold." | Not built as one report. Data exists: purchases (GRN) and stock movements per ingredient, recipe consumption from sales, order items per dish. | High | OPEN, NEW, meaning to confirm (ingredient bought vs used, or dish cost vs dish sales) |
