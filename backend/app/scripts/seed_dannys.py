...
# ---------------------------------------------------------------------------
# ITEM PHOTOS (D-33, D-35, D-38, 2026-09-25). Real Creative Commons photos, one per
# ingredient and per dish that had none (or shared the karahi photo), each picked by
# eye from Openverse by `scripts/fetch_item_photos.py`. Hosted by us in
# frontend/public/photos/ (credits in CREDITS.md there), so none can vanish mid-demo.
# Applied only where a row has no image of its own or still has the old drawn
# placeholder, so an upload from the admin screen always wins.
# ---------------------------------------------------------------------------

ITEM_PHOTOS: tuple[str, ...] = (
    "Chicken (Karahi Cut)", "Boneless Chicken", "Chicken Wings", "Mutton", "Beef Mince",
    "Beef Undercut (Local)", "Beef Ribeye (Local)", "Beef Tenderloin (Imported)", "Prawns",
    "Fish Fillet", "Tomato", "Onion", "Ginger", "Garlic", "Green Chilli", "Capsicum", "Lettuce",
    "Lemon", "Yogurt", "Fresh Cream", "Milk", "Butter", "Mozzarella Cheese", "Cheddar Slices",
    "Parmesan", "Blue Cheese", "Cooking Oil", "Salt", "Black Pepper", "Karahi Spice Mix",
    "Tikka Spice Mix", "Maida (Flour)", "Yeast", "Sugar", "Basmati Rice", "Cornflour", "Soy Sauce",
    "Chilli Sauce", "Mayonnaise", "Fettuccine Pasta", "Eggs", "Breadcrumbs", "Egg Noodles",
    "Peanuts", "Honey", "Burger Buns", "Molten Lava Cake", "Tiramisu Slice", "Cheesecake Slice",
    "Walnut Brownie Piece", "Kunafa Portion", "Frozen Fries", "Vanilla Ice Cream",
    "Espresso Beans", "Tea Leaves", "Oreo Biscuits", "Lotus Biscuits", "Soft Drink Can",
    "Mineral Water (Small)", "Caramel Syrup", "Blue Curacao Syrup", "Pineapple Juice",
    "Coconut Cream", "Karahi Masala Base", "Naan Dough", "Tikka Marinade", "Malai Marinade",
    "Garlic Mayo", "White Sauce", "Black Pepper Sauce", "Roti", "Roghni Naan", "Garlic Naan",
    "Cheese Naan", "Kunafa", "Prawn Masala",
)


def item_photo_url(name: str) -> str:
...
async def apply_item_photos(db: AsyncSession, tenant: Tenant) -> None:
    """Give every ingredient and photo-less dish its real photo (D-33, D-35, D-38).

    Replaces only an empty image, the drawn placeholder tiles of the first attempt,
    and Prawn Masala while it still carries the karahi photo. An uploaded image is
    never replaced.
    """
    karahi_photo = IMG["karahi"]
    names = set(ITEM_PHOTOS)

    def replaceable(url: str | None, name: str) -> bool:
        return (
            not url
            or url.startswith("/placeholders/")
            or (name == "Prawn Masala" and url == karahi_photo)
        )

    n = 0
    for model in (Ingredient, MenuItem):
        for row in (await db.execute(select(model).where(model.tenant_id == tenant.id))).scalars():
            if row.name in names and replaceable(row.image_url, row.name):
                row.image_url = item_photo_url(row.name)
                n += 1
    await db.flush()
