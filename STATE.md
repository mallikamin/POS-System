...
- Both deploy pipelines are touched this session: `git push origin main` for the backend + `frontend/` tablet app, and `cd storefront && npm run deploy` (Cloudflare Workers) separately for `storefront/`'s `Checkout.tsx`/`api.ts` changes. Neither has been deployed yet as of this note — see the next action below.
