...
- **Next: ask Malik/Imran for one more live retest of all 4 items**, the chime especially — real perceived loudness cannot be verified from this environment, only `tsc`/`vite build`/eslint-clean and the technique itself (square wave, layered unison, compressor, 3rd pass) per the checkpoint's reasoning.
