...
IMG: dict[str, str | None] = {
    ...
    # Was the karahi photo, so two Pakistani cards looked identical (D-33).
    "prawn_masala": None,
...
# ---------------------------------------------------------------------------
# PLACEHOLDER IMAGES (D-35, D-38, 2026-09-25). A drawn tile: the item's own food
# icon on its group's colour, never a photo of the wrong food. Generated by
# `scripts/make_placeholders.py` into frontend/public/placeholders/,
# served by nginx. Applied only where a row has no image of its own, so an
# upload from the admin screen always wins. name -> (icon, colour group)
# ---------------------------------------------------------------------------

PLACEHOLDER_ICONS: dict[str, tuple[str, str]] = {
    # Ingredients
    "Chicken (Karahi Cut)": ("🍗", "meat"), "Boneless Chicken": ("🍗", "meat"),
    "Chicken Wings": ("🍗", "meat"), "Mutton": ("🥩", "meat"), "Beef Mince": ("🥩", "meat"),
    "Beef Undercut (Local)": ("🥩", "meat"), "Beef Ribeye (Local)": ("🥩", "meat"),
    "Beef Tenderloin (Imported)": ("🥩", "meat"),
    "Prawns": ("🦐", "seafood"), "Fish Fillet": ("🐟", "seafood"),
    "Tomato": ("🍅", "veg"), "Onion": ("🧅", "veg"), "Ginger": ("🫚", "veg"),
    "Garlic": ("🧄", "veg"), "Green Chilli": ("🌶️", "veg"), "Capsicum": ("🫑", "veg"),
    "Lettuce": ("🥬", "veg"), "Lemon": ("🍋", "veg"),
    "Yogurt": ("🥛", "dairy"), "Fresh Cream": ("🥛", "dairy"), "Milk": ("🥛", "dairy"),
    "Butter": ("🧈", "dairy"), "Mozzarella Cheese": ("🧀", "dairy"), "Cheddar Slices": ("🧀", "dairy"),
    "Parmesan": ("🧀", "dairy"), "Blue Cheese": ("🧀", "dairy"),
    "Cooking Oil": ("🫒", "dry"), "Salt": ("🧂", "dry"), "Black Pepper": ("🧂", "dry"),
    "Karahi Spice Mix": ("🌶️", "dry"), "Tikka Spice Mix": ("🌶️", "dry"),
    "Maida (Flour)": ("🌾", "dry"), "Yeast": ("🌾", "dry"), "Sugar": ("🍬", "dry"),
    "Basmati Rice": ("🍚", "dry"), "Cornflour": ("🌽", "dry"), "Soy Sauce": ("🥢", "dry"),
    "Chilli Sauce": ("🌶️", "dry"), "Mayonnaise": ("🥚", "dry"), "Fettuccine Pasta": ("🍝", "dry"),
    "Eggs": ("🥚", "dry"), "Breadcrumbs": ("🍞", "dry"), "Egg Noodles": ("🍜", "dry"),
    "Peanuts": ("🥜", "dry"), "Honey": ("🍯", "dry"),
    "Burger Buns": ("🍔", "bakery"), "Molten Lava Cake": ("🍫", "bakery"),
    "Tiramisu Slice": ("🍰", "bakery"), "Cheesecake Slice": ("🍰", "bakery"),
    "Walnut Brownie Piece": ("🍫", "bakery"), "Kunafa Portion": ("🍮", "bakery"),
    "Frozen Fries": ("🍟", "frozen"), "Vanilla Ice Cream": ("🍨", "frozen"),
    "Espresso Beans": ("☕", "bev"), "Tea Leaves": ("🍵", "bev"), "Oreo Biscuits": ("🍪", "bev"),
    "Lotus Biscuits": ("🍪", "bev"), "Soft Drink Can": ("🥤", "bev"),
    "Mineral Water (Small)": ("💧", "bev"), "Caramel Syrup": ("🍯", "bev"),
    "Blue Curacao Syrup": ("🍸", "bev"), "Pineapple Juice": ("🍍", "bev"), "Coconut Cream": ("🥥", "bev"),
    # Made in-house (sub-recipes)
    "Karahi Masala Base": ("🍲", "prep"), "Naan Dough": ("🫓", "prep"), "Tikka Marinade": ("🍢", "prep"),
    "Malai Marinade": ("🍢", "prep"), "Garlic Mayo": ("🧄", "prep"), "White Sauce": ("🥣", "prep"),
    "Black Pepper Sauce": ("🥣", "prep"),
    # Dishes with no photo, or one shared with another dish (D-33, D-35)
    "Roti": ("🫓", "dish"), "Roghni Naan": ("🫓", "dish"), "Garlic Naan": ("🫓", "dish"),
    "Cheese Naan": ("🫓", "dish"), "Kunafa": ("🍮", "dish"), "Prawn Masala": ("🦐", "dish"),
}
...
def placeholder_url(name: str) -> str:
    ...
    while "--" in slug:
        slug = slug.replace("--", "-")
    return f"/placeholders/{slug}.png"
...
async def get_or_create_config(db: AsyncSession, tenant: Tenant) -> None:
    ...
    if existing is not None:
        ...
        if existing.hidden_ui_modules != HIDDEN_UI_MODULES:
            existing.hidden_ui_modules = HIDDEN_UI_MODULES
            changed = True
            print(f"  Admin menu trimmed (hidden: {HIDDEN_UI_MODULES}).")
        if changed:
            await db.flush()
        else:
    ...
    db.add(RestaurantConfig(
        ...
        hidden_ui_modules=HIDDEN_UI_MODULES,
    ))
    await db.flush()
...
async def apply_placeholder_images(db: AsyncSession, tenant: Tenant) -> None:
    """Give every ingredient and photo-less dish its drawn tile (D-35, D-38).

    Only rows with no image of their own, plus Prawn Masala while it still
    carries the karahi photo (D-33). An uploaded image is never replaced.
    """
    karahi_photo = IMG["karahi"]
    n = 0
    for ing in (await db.execute(select(Ingredient).where(Ingredient.tenant_id == tenant.id))).scalars():
        if not ing.image_url and ing.name in PLACEHOLDER_ICONS:
            ing.image_url = placeholder_url(ing.name)
            n += 1
    for mi in (await db.execute(select(MenuItem).where(MenuItem.tenant_id == tenant.id))).scalars():
        shared_photo = mi.name == "Prawn Masala" and mi.image_url == karahi_photo
        if mi.name in PLACEHOLDER_ICONS and (not mi.image_url or shared_photo):
            mi.image_url = placeholder_url(mi.name)
            n += 1
    await db.flush()
    print(f"  Placeholder images set on {n} rows.")
...
async def close_stale_kitchen_tickets(db: AsyncSession, tenant: Tenant) -> None:
    ...
    The kitchen/order sync (41270ce) stops new ones; this clears the ones left by
    the 2026-09-24 API test, which completed its order before the sync existed.
    """
    rows = (
        await db.execute(
            ...
            .where(
                KitchenTicket.tenant_id == tenant.id,
                KitchenTicket.status != "served",
                Order.status.in_(("completed", "voided")),
            )
        )
    ).scalars().all()
    for t in rows:
        t.status = "served"
        # Its own creation time, so it falls outside the board's 30-minute
        # "recently served" window and leaves the screen at once.
        t.served_at = t.served_at or t.created_at
    await db.flush()
    print(f"  Stale kitchen tickets closed: {len(rows)}.")
...
async def seed() -> None:
    async with async_session_factory() as db:
        ...
        await apply_placeholder_images(db, tenant)
