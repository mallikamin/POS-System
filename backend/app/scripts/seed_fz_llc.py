    ...
    for sdef in SUB_RECIPES:
        ...
        if produced is None:
            produced = Ingredient(
                tenant_id=tenant.id,
                name=sdef["produces"],
                # "Produced" sat one letter away from "Produce", the fresh-veg
                # category, in the same dropdown. Two unrelated things that read
                # as a typo of each other. Spotted by Malik in UAT on
                # 2026-09-06. Nothing keys on this string; the Produced BADGE
                # comes from `is_produced`, not from the category.
