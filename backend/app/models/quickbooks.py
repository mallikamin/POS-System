...
class QBConnection(BaseMixin, Base):
    ...
    company_info: Mapped[dict | None] = mapped_column(
        JSON,
        nullable=True,
        comment="Cached QB company metadata",
    )
...
class QBAccountMapping(BaseMixin, Base):
    ...
    connection_id: Mapped[uuid.UUID] = mapped_column(
        Uuid,
        ForeignKey("qb_connections.id", ondelete="CASCADE"),
        nullable=False,
    )
...
class QBEntityMapping(BaseMixin, Base):
    ...
    qb_entity_ref: Mapped[dict | None] = mapped_column(
        JSON,
        nullable=True,
        comment="Full QB ref object for API calls",
    )
    ...
    sync_hash: Mapped[str | None] = mapped_column(
        String(64),
        nullable=True,
        comment="SHA-256 of last synced state",
    )
...
class QBSyncJob(BaseMixin, Base):
    ...
    payload: Mapped[dict | None] = mapped_column(
        JSON,
        nullable=True,
        comment="Serialized data for the sync operation",
    )
    result: Mapped[dict | None] = mapped_column(
        JSON,
        nullable=True,
        comment="QB API response data on success",
    )
    ...
    error_detail: Mapped[dict | None] = mapped_column(
        JSON,
        nullable=True,
        comment="Full QB error response",
    )
...
class QBSyncLog(BaseMixin, Base):
    ...
    qb_doc_number: Mapped[str | None] = mapped_column(
        String(50),
        nullable=True,
        comment="e.g. Sales Receipt number",
    )
    amount_paisa: Mapped[int | None] = mapped_column(
        Integer,
        nullable=True,
        comment="For quick financial audit (paisa)",
    )

    batch_id: Mapped[uuid.UUID | None] = mapped_column(
        Uuid,
        nullable=True,
        comment="Groups related sync operations",
    )
...
class QBCoASnapshot(BaseMixin, Base):
    ...
    __table_args__ = (
        Index(
            "ix_qbcoa_tenant_conn_type", "tenant_id", "connection_id", "snapshot_type"
        ),
    )
